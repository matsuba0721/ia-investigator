window.onload = function () {
    function initCharacteristic(characteristic) {
        ...
        for (var i = 0; i < characteristic.weapons.length; i++) {
            $("#weapons").append(toWeaponItem(characteristic.weapons[i]));
            weaponCount++;
        }
        if (weaponCount == 0) {
            $("#weapons-table").hide();
        ...
        for (var i = 0; i < characteristic.equips.length; i++) {
            $("#equips").append(toEquipItem(characteristic.equips[i]));
            equipCount++;
        }
        if (equipCount == 0) {
            $("#equips-table").hide();
        }
        $("#money")[0].innerText = `支出レベル:${characteristic.money.pocket} 現金:${characteristic.money.cash} 資産:${characteristic.money.assets}`;
        initBackstory(characteristic.backstory);
        $("#memo")[0].innerText = characteristic.memo;
    }
    ...
    function initParameter(parameter) {
        ...
        var skill43 = characteristic.skills.find((v) => v.id == 43);
        var limit = 99 - (skill43.init + skill43.job + skill43.interest + skill43.grow + skill43.other);
    ...
    $("#characteristic-edit")[0].addEventListener("click", function (e) {
        window.location.href = "sheet.html?v=" + characteristic.id;
    });
...
if (!paramV) {
    characteristic = getInitCharacteristic(0, "");
